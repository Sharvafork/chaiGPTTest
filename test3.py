...
def generate_report(data, optimal_k, selected_features):
    response = chat(model="deepseek-r1:8b", messages=[
        ...
        f"## Executive Summary\n"
        f"The clustering analysis reveals {optimal_k} distinct clusters. The selected features used for clustering were {', '.join(selected_features)}.\n\n"
        f"## Dataset Overview\n"
        f"Dataset contains {len(data)} entries with the following key attributes: {', '.join(data.columns[:5])}...\n\n"
        f"## Cluster Analysis\n"
        f"### Cluster 1\n"
        f"- Mean Sales: {data[data['cluster'] == 0].mean().to_dict()}\n"
        f"- Market Trends: {data[data['cluster'] == 0].describe().to_dict()}\n\n"
        f"### Cluster 2\n"
        f"- Mean Sales: {data[data['cluster'] == 1].mean().to_dict()}\n"
        f"- Market Trends: {data[data['cluster'] == 1].describe().to_dict()}\n\n"
        f"## Insights and Recommendations\n"
        f"- Cluster 1: Focus on {selected_features[0]} for sustained growth.\n"
        f"- Cluster 2: Investigate anomalies in {selected_features[1]} trends.\n\n"
        f"## Conclusion\n"
        f"The clustering analysis successfully segmented the dataset into {optimal_k} clusters, providing valuable insights for strategic decision-making."}
    ])
...
iface = gr.Interface(
    ...
    outputs=gr.Textbox(label="Clustering Report", lines=20),
    title="KPI-Based Clustering Analysis",
    description="Upload a CSV file, and the system will generate a KPI-based clustering analysis with a well-formatted report."
)
