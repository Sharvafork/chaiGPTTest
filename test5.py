...
def generate_visualizations(data, selected_features):
...
def run_pipeline(file_paths, report_type):
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            visuals = generate_visualizations(clustered_data, selected_features)
    ...
    if report_type == "Collective Report":
        return generate_combined_report(clustered_datasets), visualizations
    else:
...
iface = gr.Interface(
    ...
    description="Upload CSV files and choose between generating individual reports or a collective analysis with visualizations."
)
